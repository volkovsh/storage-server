# Лабораторная 5 — Удалённое файловое хранилище (REST API по HTTP)

Сервис реализует удалённое файловое хранилище. Путь в URL задаёт логическое расположение файла/каталога в хранилище.

Пример: `http://localhost:8000/path/to/file` соответствует файлу `file` по пути `/path/to` (расширение не требуется).

## Запуск

```bash
python3 storage_server.py --host 0.0.0.0 --port 8000 --root storage_data
```

## REST API

Ниже приведён пример тестирования через **Postman**.

Базовый URL: `http://<IP_сервера>:8000/` (например, `http://192.168.1.20:8000/`)

### PUT — загрузка файла (с перезаписью)

В Postman:

- **Method**: `PUT`
- **URL**: `http://<IP_сервера>:8000/docs/hello`
- **Body**:
  - `binary` → выбрать файл (например `hello.txt`, имя файла на диске не обязано совпадать с URL)
  - или `raw` → вручную ввести текст
- **Send**

Коды:
- `201 Created` — файл создан
- `200 OK` — файл перезаписан

### PUT — копирование файла (в новое расположение)

Копирование выполняется через `PUT` на **целевой** путь с заголовком `X-Copy-From`, который указывает **исходный** путь в хранилище.

Пример:

```bash
curl -i -X PUT "http://localhost:8000/path/to/file.txt" \
  -H "X-Copy-From: /path/to/another/file2.txt"
```

Коды:
- `201 Created` — файл создан по новому пути
- `200 OK` — файл по целевому пути перезаписан (или копирование в самого себя — no-op)
- `404 Not Found` — исходный файл не найден

### GET — получение файла

В Postman:

- **Method**: `GET`
- **URL**: `http://<IP_сервера>:8000/docs/hello`
- **Send**

Ожидаемо: `200 OK`, в **Body** содержимое файла.

### GET — список файлов каталога (JSON)

В Postman:

- **Method**: `GET`
- **URL**: `http://<IP_сервера>:8000/docs/`
- **Send**

Ответ: JSON со списком `items` (name/type/size/mtime). Открытие каталога также работает в браузере.

### HEAD — информация о файле (только заголовки)

В Postman:

- **Method**: `HEAD`
- **URL**: `http://<IP_сервера>:8000/docs/hello`
- **Send**

Заголовки:
- `Content-Length` — размер в байтах
- `Last-Modified` — дата последнего изменения

### DELETE — удаление файла/каталога

В Postman:

- **Method**: `DELETE`
- **URL**:
  - файл: `http://<IP_сервера>:8000/docs/hello`
  - каталог: `http://<IP_сервера>:8000/docs/`
- **Send**

Коды:
- `204 No Content` — удалено
- `404 Not Found` — не найдено

## Примечания

- Каталоги создаются автоматически при `PUT`.
- Для чтения больших файлов используется потоковая передача (без загрузки файла целиком в память).
- Защита от выхода за пределы корня хранилища: `..` не допускается.

